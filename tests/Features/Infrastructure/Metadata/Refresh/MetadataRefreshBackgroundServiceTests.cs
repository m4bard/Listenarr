...
public class MetadataRefreshBackgroundServiceTests : BaseTests
{
    ...
    public void ApplicationSettings_ShipTheTimidBudget_AndShipItOn()
    {
        // 60 an hour with a one-second floor is deliberately conservative: the provider
        // publishes no limit. On by default, which is only safe because an upgraded row does not
        // arrive due: the startup backfill gives every existing book a timestamp taken from its
        // own dates, so the library enters the queue spread out and in an order.
