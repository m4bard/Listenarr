...
namespace Listenarr.Tests.Features.Application.Audiobooks.Matching
{
    ...
    ///
    /// The per-profile "upgrades allowed" flag now exists, and its interaction with a blank
    /// cutoff is decided in QualityMatcher rather than here: ResolveCutoff reports a profile
    /// with upgrades off as blank, so "blank cutoff with upgrades off" reads the same as
    /// "blank cutoff alone", True. A test pinning False for either is pinning a state this
    /// change retired.
    ///
