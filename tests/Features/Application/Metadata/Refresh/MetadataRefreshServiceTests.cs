...
public class MetadataRefreshServiceTests : BaseTests
{
    ...
    /// <summary>
    /// Records when the book's exclusive section was entered and left, and when the budget was
    /// charged, so a test can assert the order of the two.
    /// </summary>
    private sealed class ExclusiveSectionLog
    {
        private int _next;

        public int? EnteredAt { get; private set; }
        public int? LeftAt { get; private set; }

        public int Next() => Interlocked.Increment(ref _next);

        public void Entered() => EnteredAt = Next();

        public void Left() => LeftAt = Next();
    }

    /// <summary>A budget that notes when each charge happened against the shared sequence.</summary>
    private sealed class SequencedBudget(ExclusiveSectionLog log) : IMetadataRefreshBudget
    {
        public int RequestsSpent { get; private set; }

        public List<int> ChargedAt { get; } = [];

        public Task<bool> ChargeAsync(CancellationToken cancellationToken)
        {
            RequestsSpent++;
            ChargedAt.Add(log.Next());
            return Task.FromResult(true);
        }
    ...
    private static MetadataRefreshService CreateService(
        ...
        ExclusiveSectionLog? exclusiveSectionLog = null)
    {
    ...
    [Fact]
    [Trait("Scenario", "AnUnaskableBookIsUnusableNotMissing")]
    public async Task RefreshAsync_ReturnsUnusable_WhenNoIdentifierCouldBeAskedAbout()
    {
        // An ISBN and no resolver wired: there is an identifier, so this is not the no-identifier
        // case, but nothing is ever asked. NotFound would claim a provider verdict that no
        // provider gave, and Skipped would claim the book carries no identifiers, which a caller
        // reporting it to a person repeats back to them.
        ...
        Assert.Equal(MetadataRefreshOutcome.Unusable, result.Outcome);
    ...
    public async Task RefreshAsync_Defers_AfterTwoRetries_AndReportsTheThrottleSignal()
    {
        ...
        // Once, on the first 429 of the book, however many the book goes on to see. Signalling
        // per attempt compounds: three over one book take sixty an hour down to seven before
        // the second book is reached, and the budget is meant to halve for the rest of the
        // cycle, singular.
    ...
    public async Task RefreshAsync_NarrowsTheRun_WhenA429IsFollowedByASuccessfulRetry()
    {
        ...
        // The run is not fine. The provider asked for less traffic and named a wait, and a
        // retry that happened to succeed does not withdraw either. Narrowing only at the retry
        // ceiling would drop both and send the walk straight back at a host that just said
        // no.
        Assert.Equal(1, budget.ThrottleSignals);
    ...
    [Fact]
    [Trait("Scenario", "AnIdentifierThatWillNotNormalizeIsUnusable")]
    public async Task RefreshAsync_ReturnsUnusable_WhenTheOnlyAsinWillNotNormalize()
    {
        // A stored ASIN that the normalizer rejects. The book has an identifier, so this is not
        // the no-identifier case; nothing is ever asked, so it is not a provider verdict either.
        var book = BookWithAsin("not an asin at all");
        var metadata = new Mock<IAudiobookMetadataService>();
        var budget = new CountingBudget();
    ...
    [Fact]
    [Trait("Scenario", "TheCoverChargeIsTakenOutsideTheBooksLock")]
    public async Task RefreshAsync_ChargesForTheCover_BeforeItTakesTheBooksExclusiveLock()
    {
        var metadata = new Mock<IAudiobookMetadataService>();
        metadata
            .Setup(m => m.GetMetadataAsync("B0COVERART", "us", false))
            .ReturnsAsync(new AudiobookMetadataEnvelope(
                new AudibleBookResponse
                {
                    Asin = "B0COVERART",
        ...
        var log = new ExclusiveSectionLog();
        var budget = new SequencedBudget(log);
        var service = CreateService(BookWithAsin("B0COVERART"), metadata, exclusiveSectionLog: log);

        var result = await service.RefreshAsync(1, budget, CancellationToken.None);

        Assert.Equal(MetadataRefreshOutcome.Updated, result.Outcome);

        // Two charges: the provider lookup and the cover. Both before the exclusive section.
        //
        // A charge can wait, for the spacing floor or for a whole Retry-After the provider
        // named. Taken inside the section, that wait is held against the book's own lock, and
        // every import, move and rename queued against that book waits behind a rate limiter
        // it has no part in.
        Assert.Equal(2, budget.RequestsSpent);
        Assert.NotNull(log.EnteredAt);
        Assert.All(budget.ChargedAt, at => Assert.True(
            at < log.EnteredAt,
            $"A charge at step {at} happened inside the exclusive section, entered at step {log.EnteredAt}"));
    }

    [Fact]
    [Trait("Scenario", "NoCoverMeansNoCoverCharge")]
    public async Task RefreshAsync_ChargesOnlyForTheLookup_WhenTheProviderOfferedNoCover()
    {
        // The control for the test above. Charging before the lock must not become charging
        // unconditionally: a book the provider has no image for costs one request, not two.
        var metadata = new Mock<IAudiobookMetadataService>();
        metadata
            .Setup(m => m.GetMetadataAsync("B0NOCOVERX", "us", false))
            .ReturnsAsync(new AudiobookMetadataEnvelope(
                new AudibleBookResponse { Asin = "B0NOCOVERX", Title = "Has No Cover" },
                "Audible",
                "https://example.invalid/product"));
        ...
        var result = await CreateService(BookWithAsin("B0NOCOVERX"), metadata, exclusiveSectionLog: log)
            .RefreshAsync(1, budget, CancellationToken.None);
