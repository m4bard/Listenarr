...
- **Series-only advanced search returning no results:** Searching by series name alone (e.g. "Fjällbacka Mysteries") in the Advanced search returned no results because the query builder produced an empty search string when no title/author/narrator was provided. Fixed by using the series name as a title query when it's the only search parameter.
