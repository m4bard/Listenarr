...
namespace Listenarr.Domain.Search
{
    ...
    public static class TorznabIndexerFlagParser
    {
        ...
        public static List<string> Parse(IEnumerable<KeyValuePair<string, string>> attributes)
        {
            ...
            var uploadFactor = 1d;
            var seenDownloadFactor = false;
            var seenUploadFactor = false;
            var tags = new List<string>();

            // Torznab feeds repeat attr elements routinely, which is how "tag" carries more than
            // one value. For the two factors the first occurrence decides, the way Sonarr reads a
            // single-valued attribute with FirstOrDefault
            // (src/NzbDrone.Core/Indexers/Torznab/TorznabRssParser.cs, TryGetTorznabAttribute) and
            // then falls back to the default if that one does not parse.
            foreach (var attribute in attributes)
            {
                switch (attribute.Key?.ToLowerInvariant())
                {
                    case "downloadvolumefactor":
                        if (!seenDownloadFactor)
                        {
                            seenDownloadFactor = true;
