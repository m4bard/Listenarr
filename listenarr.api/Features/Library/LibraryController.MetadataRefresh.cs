...
public partial class LibraryController
{
    ...
    /// <returns>Accepted with a run id, or Conflict naming the run already in flight.</returns>
    [HttpPost("refresh-metadata")]
    public async Task<IActionResult> StartMetadataRefresh(
        [FromBody] MetadataRefreshRequest? request,
        CancellationToken cancellationToken = default)
    {
        return await _metadataRefreshWorkflow.StartAsync(request, cancellationToken);
    }
