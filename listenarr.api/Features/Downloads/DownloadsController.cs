...
public class DownloadsController : ControllerBase
{
    ...
    public async Task<ActionResult> ClearFailedDownloads([FromQuery] bool removeFromClient = true)
    {
        ...
        {
            var all = await _downloadRepository.GetAllAsync();
            // IsTerminalFailure() is Failed or ImportBlocked, which is the same set this branch
            // spelled out inline. The helper is kept so the bulk clear and everything else that
            // asks "is this download finished and failed" keep answering from one definition.
