...
- API note: primary Discord bot metadata calls now use `/api/metadata/{asin}` (and `/api/metadata/audible/{asin}`); legacy `/api/search/metadata` routes are deprecated and may be removed.
