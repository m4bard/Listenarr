...
public sealed partial class MetadataRefreshService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // A cover is a request to the provider's CDN like any other, one per updated book, so it
        // is counted and spaced with the rest: a run at sixty an hour that updated every book it
        // touched would otherwise make a hundred and twenty. The charge was taken by the caller
        // before it entered this exclusive section, because waiting for a grant here would hold
        // the book's lock against every import and move queued behind it. A refusal is the run's
        // window closing, and the metadata is already written, so the book keeps the provider's
        // own image URL exactly as it does when the move fails.
        //
        // The charge is taken even when the move turns out to be served from cache and no
        ...
        if (coverChargeGranted && !string.IsNullOrWhiteSpace(metadata.ImageUrl))
        {
