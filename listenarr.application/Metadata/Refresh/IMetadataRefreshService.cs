...
public enum MetadataRefreshOutcome
{
    ...
    /// <summary>
    /// Nothing was asked because there was nothing to ask with: no ASIN and no ISBN at all.
    /// Stamped so it leaves the queue head. No provider had any part in it.
    /// </summary>
    Skipped,

    /// <summary>
    /// Identifiers were present and none of them could be used: every one failed to normalize,
    /// or the only ones left were ISBNs with no resolver wired. Nothing was asked, so like
    /// <see cref="Skipped"/> this is settled locally and stamped. It is told apart from
    /// <see cref="Skipped"/> because a caller reporting it to a person has to say something
    /// different: "this book carries no identifiers" is untrue of it.
    /// </summary>
    Unusable,
