...
| `ScanBackgroundService` | `ScanJobProcessor` | Consumes scan jobs, reconciles audiobook files/metadata for the audiobook library path, and records terminal move-scan history before changing the in-memory scan state. | Must not move audiobook roots or import download payloads. | In-memory scan jobs can be requeued from failed/completed/queued status. The hosted service also retries durable move-scan handoffs that were recorded before an immediate enqueue failed. | `ScanQueueService` serializes enqueue, replay, requeue, and terminal status changes; it dedupes queued/processing jobs by audiobook and path, and move-correlated terminal history prevents replay after completion or failure. | Broadcasts library updates after reconciliation. Move completion records a durable `ScanQueued` history handoff that `MoveScanHandoffRecoveryService` can dispatch after restart or transient enqueue failure. |
| `MoveBackgroundService` | `MoveJobProcessor` | Owns audiobook filesystem relocation and move-job transitions `Queued/RetryScheduled -> Running -> Completed/Failed/NeedsAttention`, including manifest checkpoints, filesystem ownership evidence, metadata rebasing, artifact cleanup, and completion handoffs. | Must not import downloads or claim scan execution ownership. It may request a post-move scan only through the durable scan handoff. | Transient filesystem and completion-handoff failures use persisted exponential backoff with jitter and a bounded automatic retry count. Exhaustion transitions to `NeedsAttention`; explicit manual requeue resets the retry budget while preserving lease generation fencing. | `MoveQueueService` uses async persistence plus a database-unique active deduplication key for audiobook and requested path. Leases and generations fence every filesystem mutation and durable checkpoint; structured markers and manifests make replay idempotent, and terminal transitions release the key. | Records move history and a durable post-move `ScanQueued` handoff before marking the move complete, then broadcasts the updated audiobook. |
