...
-->
<template>
  <section v-if="seriesRows.length > 0" class="author-series-section">
    <div class="author-series-header">
      <span class="section-title">Series</span>
      <span class="section-count">{{ seriesRows.length }}</span>
      <button
        type="button"
...
const seriesRows = computed(() =>
  seriesGroups.value.map((group) => ({
    ...
    panelId: authorSeriesElementId('author-series-books', group.key),
    noteId: authorSeriesElementId('author-series-monitor-note', group.key),
...
async function fetchMonitoringStatus(group: AuthorSeriesGroup): Promise<MonitoredSeries | null> {
  try {
    ...
    // The endpoint resolves a series by name, while this section knows a series by its ASIN.
