...
async function onCopy() {
  ...
  try { await navigator.clipboard.writeText(props.apiKey) } catch (e) { console.error('Clipboard write failed', e) }
}
...
async function onRegenerate() {
  ...
  try {
    if (!props.apiKey) {
      ...
      if (res?.apiKey) { emit('update:apiKey', res.apiKey); await navigator.clipboard.writeText(res.apiKey) }
    } else {
      const res = await apiService.regenerateApiKey()
      if (res?.apiKey) { emit('update:apiKey', res.apiKey); await navigator.clipboard.writeText(res.apiKey) }
    }
  } catch (e) { console.error('Failed to (re)generate API key', e) }
}
</script>

<style scoped>
.api-key-control { display:flex; gap:0.5rem; align-items:center }
.controls { display:flex; gap:0.5rem }
.copy-btn, .regen-btn { padding:0.5rem 0.75rem; border-radius:6px; border:none; cursor:pointer }
