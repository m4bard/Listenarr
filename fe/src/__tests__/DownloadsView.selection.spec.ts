...
// The view takes showConfirm from confirmService, which is where the rest of
// this page's confirmations already come from. useConfirm only re-exports the
// same singleton, so mocking that module would leave the real one in place and
// the run would hang on a promise nothing resolves.
