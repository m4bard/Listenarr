...
function mountTab(tabBooks: AuthorSeriesBook[] = books) {
  ...
  return mount(AuthorSeriesTab, {
    props: { books: tabBooks, region: 'us', language: 'english' },
    global: { plugins: [router] },
  })
}

function expandedPanels(wrapper: ReturnType<typeof mountTab>) {
  return wrapper
    .findAll('.author-series-books')
    .filter((panel) => (panel.element as HTMLElement).style.display !== 'none')
}

describe('AuthorSeriesTab', () => {
  beforeEach(() => {
  ...
  it('renders one collapsed row per distinct series with the header count', async () => {
    ...
    expect(wrapper.find('.section-count').text()).toBe('3 series')
  ...
  it('gives every panel a valid HTML id that its disclosure control points at', async () => {
    ...
    expect(panelIds).toEqual([
      'author-series-tab-books-asin-b0series02',
      'author-series-tab-books-asin-b0series01',
      'author-series-tab-books-name-tidewater-papers',
    ])
  ...
  it('shows an empty state instead of series cards when no book carries a series', () => {
    const wrapper = mountTab([{ key: 'library-9', title: 'Standalone', inLibrary: true }])
    expect(wrapper.find('.author-series-card').exists()).toBe(false)
    expect(wrapper.find('.author-series-list').exists()).toBe(false)
  ...
  it('expands and collapses every series from the tab header', async () => {
    const wrapper = mountTab()
    await flushPromises()
  ...
  it('asks for monitoring status only for series that have an ASIN', async () => {
    ...
    expect(noteId).toBe('author-series-tab-monitor-note-name-tidewater-papers')
