...
namespace Listenarr.Infrastructure.Persistence.Configurations
{
    ...
    public class ApplicationSettingsConfiguration : IEntityTypeConfiguration<ApplicationSettings>
    {
        ...
        private static ValueComparer<List<CustomScriptConfiguration>?> CustomScriptListComparer() =>
            new ValueComparer<List<CustomScriptConfiguration>?>(
                (c1, c2) => JsonSerializer.Serialize(c1, (JsonSerializerOptions?)null) == JsonSerializer.Serialize(c2, (JsonSerializerOptions?)null),
                c => c == null ? 0 : JsonSerializer.Serialize(c, (JsonSerializerOptions?)null).GetHashCode(),
        ...
        public void Configure(EntityTypeBuilder<ApplicationSettings> builder)
        {
            ...
            // CustomScripts stored as JSON
            builder.Property(e => e.CustomScripts)
                .HasConversion(
                    v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                    v => string.IsNullOrWhiteSpace(v)
                        ? null
                        : JsonSerializer.Deserialize<List<CustomScriptConfiguration>>(v, (JsonSerializerOptions?)null)
                );
            builder.Property(e => e.CustomScripts)
